 ...
 */
export class TopologicalSortStrategy implements SortingStrategy<StoryblokResource> {
	/**
	...
	sort(resources: StoryblokResource[]): StoryblokResource[] {
		...
		const parentDependencies: [number | null, number][] = resources.map((resource) => {
		...
		return sortedIds.map((id) => idToResourceMap.get(id)).filter((item) => item !== undefined)
	}
}
